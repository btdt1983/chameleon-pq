            ...
            tokio::select! {
                ...
                _ = tx_diag.tick() => {
                    if t_pkts > 0 {
                        ...
                        info!(
                            "TX-DIAG/2s: {} pkts ({}/s) from TUN | {} flushes, {:.1} pkts/flush | from_tun backlog max {}/512 | send_tx free {}/{}",
                            t_pkts, t_pkts / 2, t_flushes, avg_flush, t_qmax,
                            send_tx.capacity(), send_tx.max_capacity(),
                        );
                    }
